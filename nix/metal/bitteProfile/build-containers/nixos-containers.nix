...
let
  ...
  startScript = cfg:
  let
    closureInfo = pkgs.closureInfo { rootPaths = { inherit (cfg.config.system.build) toplevel; }; };
    closure = lib.fileContents "${closureInfo}/store-paths";
    storePaths = lib.splitString "\n" closure;
    bindMounts = map (s: "${s}:${s}") storePaths;
    bindFlags = if cfg ? config then
    toString (lib.cli.toGNUCommandLine {} { bind = bindMounts ++ [
      "${closureInfo}/registration:/etc/nix-db-merge/registration"
      "${closureInfo}/store-paths:/etc/nix-db-merge/store-paths"
    ]; })
    else
    "";
  in
                    ...
                    let
                      extraConfig = { options, ... }: {
                        ...
                        config = {
                          ...
                          system.activationScripts.registration.text = ''
                            ${pkgs.writeShellApplication {
                              name = "merge-nix-db";
                              runtimeInputs = [pkgs.ruby pkgs.nix];
                              text = ''
                                exec ruby ${./scripts/merge.rb}
                              '';
                            }}/bin/merge-nix-db
                          '';
